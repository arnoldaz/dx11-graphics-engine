...
fn main() {
    ...
    let mut application = WindowApplication::new(&window).unwrap();

    let mut window_ui = WindowUi::new(&window, &application.device, &application.device_context).expect("Window UI failed to init");
